...
25. Playerが中間ランキング表示中にページをリロードしても、同じランキング画面に戻る。
26. Playerが「次へ」を押さないままHostが次ステージへ進めても、Playerはランキング画面のまま残る。
27. その後Playerが「次へ」を押すとHostの現在フェーズへ追従し、新ゲーム開始後は古いランキング保持が復元されない。
