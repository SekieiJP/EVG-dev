...
13. UUID復元で、現在ゲーム外の過去UUIDから名前とSkill履歴を復元できる。
14. 参加登録前Player画面と未認証Host画面で定期ポーリングが発生しない。
15. Apps Script実行ログにpath、role、uuid、処理時間、エラー種別がJSONで残る。
16. `/api/status` に現在の `roomVersion` を渡すと `unchanged` が返り、フルルームが返らない。
17. `/api/host/commit-result` がHostブラウザで計算した結果roomを保存し、バージョン不一致と二重集計を拒否する。
18. 同一端末Screen同期モードで、Host操作後にGASポーリングなしでScreenが更新される。
19. 参加登録、Host認証、投票、戦績取得中に読み込み表示が出て、通信中は追加操作できない。
20. 同一ゲーム・同一ステージ・同一結果数では、Player本人の個人戦績がローカルキャッシュから表示される。
21. Host/Screen画面の起動時に、Player UUIDが保存済みでも `/api/player/restore` が送られない。
22. `v=player`、`view=player`、URLパラメータなしのPlayer入口でHost/Screenタブに遷移できない。
23. 認証済みHost画面で、締切カウントダウンと移動中演出終了後に自動集計される。
24. 結果発表演出の最終階到達後、Hostがランキングへ進める前にPlayerで個人結果を表示できる。
25. Player入力欄フォーカス中に状態ポーリング再描画で入力が消えない。
26. History画面で全員ランキングは見えるが、個人詳細は本人UUIDの端末だけ表示される。
27. GAS通信失敗時に最大3回まで短いリトライが行われ、通信ログに `api.retry` が残る。
28. Playerが中間ランキング表示中にページをリロードしても、同じランキング画面に戻る。
29. Playerが「次へ」を押さないままHostが次ステージへ進めても、Playerはランキング画面のまま残る。
30. その後Playerが「次へ」を押すとHostの現在フェーズへ追従し、新ゲーム開始後は古いランキング保持が復元されない。
31. Player最終結果画面で「次ゲームへ」を押すまで新ゲームへ遷移せず、押下後にHostの新ゲームへ追従する。
